...
async def reload_cogs_command(ctx):
    ...
    try:
        # 重新載入 conversation cog
        if 'cogs.conversation' in bot.extensions:
            await bot.reload_extension('cogs.conversation')
            logger.info("✅ conversation cog 重新載入完成")
        else:
            await bot.load_extension('cogs.conversation')
            logger.info("✅ conversation cog 載入完成")
...
async def load_extensions():
    ...
    try:
        ...
        logger.info("已載入 music cog")
    except Exception as e:
        logger.error(f"載入 music cog 時出錯: {e}", exc_info=True)
